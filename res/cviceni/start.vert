...
vec3 function(vec2 uv){
	...
	switch(functionType){
		case 1:
			posit = vec3(uv.x, uv.y, 0.0);
			posit -= 0.5; 		// zmena na <-0.5, 0.5>
			posit *= 2;		// zmena na <-1.0, 1.0>
			posit *= 10; 
			break;
		case 2:
...
void main() {
	...
	positionBack = function(backPos);
		
	if(functionType>4 && functionType < 10){
		texCoord = vec2(inPosition.y, inPosition.x);					
	} else {
		texCoord = inPosition;
